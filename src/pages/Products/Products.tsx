...
const Products = ()=>{
    ...
    useEffect(()=>{
        if(selectedCategories.length===0)
            ...
            Promise.all(
                selectedCategories.map((v)=>getProducts(Number(v)))
    ...
    return (
        <BlockContainer className={styles.blockContainer}>
            <div className={styles.products}>
                ...
                <MultiDropdown generateValueElement={() => {
                    return 'Filter'
                }} options={options ? options : []} value={selectedCategories.map(categoriesToOptions)}
